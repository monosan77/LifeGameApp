...
export default function Gameboard({ roomId }: any) {
  const [playerPositions, setPlayerPositions] = useState<number[]>([0, 0, 0]); // 3人のプレイヤーの位置を管理（初期値は全員0）
  ...
  useEffect(() => {
    ...
    channel.bind('result-dice', async function (resultDice: number | null) {
      if (resultDice) {
        ...
        newPosition[currentPlayer] = newPosition[currentPlayer] + resultDice;
        setPlayerPositions(newPosition);

        await getNextPlayer(newPosition);
      }
    });
  ...
  }, [CLUSTER, KEY, currentPlayer, playerPositions, roomId]);

  // ダイスを振る関数
  async function rollDice() {
    const res = await fetch(
      `http://localhost:3000/api/game/dice?roomId=${roomId}`
    );
  ...
  async function getNextPlayer(newPosition: number[]) {
    const res = await fetch(
      `http://localhost:3000/api/game/taun-change?roomId=${roomId}`,
      {
        method: 'POST',
        headers: {
